...
#define UUPICKER_MAXDATE 2021
#define UUPICKER_MINDATE 2015

#define UUPICKER_MONTH 12
#define UUPICKER_HOUR 24
#define UUPICKER_MINUTE 60
#define MEMBERCOUNT 30
...
- (void)drawRect:(CGRect)rect
{
    ...
    memberCountArray = [self ishave:memberCountArray];
    
    //赋值
    for (int i=2; i<=MEMBERCOUNT; i++) {
        NSString *num = [NSString stringWithFormat:@"%d",i];
        [memberCountArray addObject:num];
    }
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView
{
    ...
    if (self.datePickerStyle == TJMemberCountStyle){
        if (isIOS7) {
            [self creatValuePointXs:@[@"170"]
                          withNames:@[@"人"]];
        }
        return 1;
    }
